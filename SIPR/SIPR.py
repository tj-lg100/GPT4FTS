...
class SIPR:
  def __init__(self, n_clusters, l_min=8, l_max=16, gamma=.5):
  ...
  def plot_centroids(self, iter):
    n_rows = 3  
    n_cols = math.ceil(self.n_clusters / n_rows)  
    ...
    for j in range(len(self.centroids), len(axs)):
  ...
  def save_results(self, new_loss, dataname, init_strategy, barycenter):
    if new_loss<=self.min_loss:
      ...
      dict_init = {'kmeans++': 'kmpp',
                  ...
                  'random_noise': 'rn',
                  'reference':'ref'}
      filename = f"sipr_{dataname}_k{self.n_clusters}_l{self.l_min}-{self.l_max}_{barycenter[:4]}_{dict_init[init_strategy]}"
